...
static const CGFloat kLabelFontSize = 10.f;
static const CGFloat kDetailsLabelFontSize = 9.f;
...
- (void)layoutSubviews {
	...
	CGSize _maxSize = CGSizeMake(maxWidth, remainingHeight);
	CGSize detailsLabelSize = MB_MULTILINE_TEXTSIZE(detailsLabel.text, detailsLabel.font, _maxSize, detailsLabel.lineBreakMode);
    ...
    if (totalSize.width > self.maxSize.width) {
        totalSize.width = self.maxSize.width;
    }
    if (totalSize.height > self.maxSize.height) {
        totalSize.height = self.maxSize.height;
    }
